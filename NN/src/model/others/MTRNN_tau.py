...
class MTRNN(nn.Module):  # [TODO]cannot use GPU now
    def __init__(
        ...
        self.attention = torch.nn.Sequential(
            torch.nn.Linear(layer_size["in"] + layer_size["cs"], 10),
            torch.nn.ReLU(),
    ...
    def forward(self, x):  # x.shape(batch,x)
        ...
        attention_in = torch.cat([x, self.cs_state], axis=1)
        self.cf_tau = self.attention(attention_in)
