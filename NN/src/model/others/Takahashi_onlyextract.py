...
class AttentionMTRNN(torch.nn.Module):  # [TODO]cannot use GPU now
    ...
    def forward(self, x):
        ...
        extracted = torch.cat([position, tactile, image], axis=1)
        attention_in = torch.cat([tactile, image], axis=1)
        ...
        return self.mtrnn(extracted * self.attention_map)
