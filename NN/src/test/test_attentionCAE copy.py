...
combined_imgs = []
fig = plt.figure(dpi=50, figsize=(10, 2))
count = 1
for i, (inputs, labels) in enumerate(testloader):
    ...
    for img, output_img, mask in zip(
        ...
        plt.subplot(1, 3, 1)
        plt.imshow(np.asarray(input_pil))
        plt.axis("off")

        plt.subplot(1, 3, 2)
        plt.imshow(np.asarray(output_pil))
        plt.axis("off")
        ...
        gray = gray.resize((128, 96), Image.NEAREST)
        gray_arr = np.asarray(gray)
        sns.heatmap(gray_arr)
        plt.savefig(RESULT_DIR + "{}.jpeg".format(count))
        count += 1
        fig.clf()
        """
        fig.canvas.draw()
