...
from model.Takahashi_onlyextract import AttentionMTRNN as MTRNN

if __name__ == "__main__":
    argnum = len(sys.argv)

    if argnum == 4:
        _, name, is_sigmoid, load_path = sys.argv
    elif argnum == 3:
        _, name, is_sigmoid = sys.argv
        load_path = ""
    #     name = str(name)
    #     in_size = int(in_size)
    ...
    MODEL_DIR = MODEL_BASE + "MTRNN/1123/{}/".format(name)
    ...
    if is_sigmoid == "sigmoid":
        activate = torch.nn.Sigmoid()
    elif is_sigmoid == "softmax":
