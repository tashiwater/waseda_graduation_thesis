...
net = MTRNN(
    ...
    c_size={"io": 4, "cf": 4, "cs": 4},
    tau={"tau_io": 2, "tau_cf": 5, "tau_cs": 50},
)

### modelをロード
model_path = MODEL_DIR + "20200708_194551_3500.pth"
checkpoint = torch.load(model_path)
net.load_state_dict(checkpoint["model"])
# header = (
#     ["position{}".format(i) for i in range(7)]
#     + ["torque{}".format(i) for i in range(7)]
#     + ["tactile{}".format(i) for i in range(12)]
#     + ["image{}".format(i) for i in range(20)]
# )
# header = ["sint", "sin2t"]
...
for j, (one_batch_inputs, one_batch_labels) in enumerate(dataloader):
    ...
    outputs = torch.empty_like(labels_transposed)
    io_states = []
    cf_states = []
    cs_states = []
    ...
    connected_data = np.block(
        [np_input, np_output, io_states, cf_states, cs_states, cf_pca, cs_pca]
    )
    # df_input = pd.DataFrame(data=np_input, columns=header)
    # df_input.to_csv(RESULT_DIR + "input{:02}.csv".format(j + 1), index=False)
    header = (
