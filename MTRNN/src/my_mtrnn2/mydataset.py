...
class CsvDataSet(torch.utils.data.Dataset):
    def __init__(self, dir_path):
        ...
        for i, path in enumerate(self._paths):
            # print(path)
            x = torch.from_numpy(np.loadtxt(path, delimiter=",")[:750]).float()
            # print(x.shape)
            self._datas.append(x.view(-1, 1, x.shape[1]))
...
class EasyDataSet(torch.utils.data.Dataset):
    ...
    def target_func(self, t):
        return torch.tensor([math.sin(t), math.sin(2 * t)])
    ...
    def make_data(self, start, data_num, step, noise, target_func):
        ...
        target_size = len(t_list), len(self.target_func(0))
        rand_list = torch.randn(target_size) * noise
        for t, rand in zip(t_list, rand_list):
            x = list(target_func(t) + rand)
            x2 = list(target_func(t + step))
            ret.append(x)
            ret2.append(x2)
        x = torch.tensor(ret).view(-1)
        return torch.tensor(ret).view(-1, 1, 2), torch.tensor(ret2).view(-1, 1, 2)
